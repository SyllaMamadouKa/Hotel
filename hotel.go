...
func getReservation(reservationID int) (Reservation, error) {
	...
	var reservation Reservation
	err = row.Scan(&reservation.ID, &reservation.DateEntree, &reservation.DateSortie, &reservation.PrixTotal)
	if err != nil {
		if err == sql.ErrNoRows {
...
func updateReservationHandler(w http.ResponseWriter, r *http.Request) {
	...
	query := "UPDATE reservation SET dateEntree=?, dateSortie=?, prixTotal=? WHERE id=?"
...
func getChambre(chambreID int) (Chambre, error) {
	...
	query := "SELECT * FROM chambre WHERE id=?"
...
func updateChambreHandler(w http.ResponseWriter, r *http.Request) {
	// Récupère l'ID de la chambre à mettre à jour depuis les paramètres de la requête
	...
	// Récupère les données de la chambre à partir du corps de la requête
	reqBody, err := ioutil.ReadAll(r.Body)
	if err != nil {
		http.Error(w, "Failed to read request body", http.StatusInternalServerError)
		return
	...
	// Ouvre une connexion à la base de données MySQL
	db, err := sql.Open("mysql", "root:@tcp(127.0.0.1:3306)/geschambre")
	if err != nil {
		http.Error(w, "Failed to connect to database", http.StatusInternalServerError)
		return
	}
	...
	query := "UPDATE chambre SET etat=?,  classe=? WHERE id=?"
